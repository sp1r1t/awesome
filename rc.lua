...
-- names = {"", "", "", "", "", ""};
icons = {"world.svg", "coding64.png", "edit64.png", "terminal.png", "", "", "", "", "", "", "mail.png"}
...
myawesomemenu = {{"manual", terminal .. " -e man awesome"}, {"edit config", editor_cmd .. " " .. awesome.conffile},
...
mytextclock:connect_signal("button::press", function()
    local client = awful.spawn.raise_or_spawn('thunderbird', {
        class = 'thunderbird'
    })
end)
...
mytaglist.buttons = awful.util.table.join(awful.button({}, 1, awful.tag.viewonly),
    awful.button({modkey}, 1, awful.client.movetotag), awful.button({}, 3, awful.tag.viewtoggle),
    awful.button({modkey}, 3, awful.client.toggletag), awful.button({}, 4, function(t)
...
for s = 1, screen.count() do
    ...
    mylayoutbox[s]:buttons(awful.util.table.join(awful.button({}, 1, function()
    ...
    mytaglist[s] = awful.widget.taglist({
        ...
        widget_template = {
            ...
            create_callback = function(self, c3, index, objects) -- luacheck: no unused args
                ...
                self:connect_signal("button::press", function(widget, x, y, button)
                    if button == 1 then
    ...
    vicious.register(memwidget, vicious.widgets.mem, function(memwidget, args)
        local output = "M: "
        local m = args[1]
        if m > 90 then
    ...
    vicious.register(cpuwidget, vicious.widgets.cpu, function(cpuwidget, args)
        local output = "C: "
    ...
    vicious.register(notmuchwidget, vicious.widgets.notmuch, function(widget, args)
        return tostring(args.count)
    end, 100, "tag:unread")

    notmuchicon = wibox.widget.imagebox(beautiful.icon.mail)
    notmuchicon:connect_signal("button::press", function(c)
